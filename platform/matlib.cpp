...
void MatLib::corrupt(const string &filename)
{
...
int MatLib::readc(FILE *in, const string &filename)
{
	int c = fgetc(in);
...
void MatLib::get_mtl(FILE *in, const string &filename)
{
	if (m_cur != NULL)
...
void MatLib::get_att(FILE *in, const string &filename, const string &att)
{
	float r, g, b;
...
void MatLib::get_shininess(FILE *in, const string &filename)
{
	float n;
	if (fscanf(in, "%f", &n) < 1)
		corrupt(filename);
...
void MatLib::open(const string &filename)
{
	...
	FILE *in;
	int ch;
	const string fullname = "materials/" + filename;
	if ((in = fopen(fullname.c_str(), "r")) == NULL)
	{
